    ...
    Artifact *artifact, FileResourceBase *file, const char *fileTags)
{
    ScanResult result;
    if (m_plugin) {
        const ScannerScanResult scanResult
            = m_plugin->scan(file->filePath(), fileTags, artifact->properties->value());
    ...
    Artifact *artifact, const FileResourceBase *fileToScan, const PrivateScriptFunction &script)
{
    ...
    if (!JS_IsObject(result) || JS_IsArray(result) || JS_IsError(result) || JS_IsRegExp(result)) {
        throw ErrorInfo(Tr::tr("Scan script must return an array of dependency paths or an object "
                               "with scannerProperties."),
                        script.location());
    }
    ...
    if (!JS_IsUndefined(searchPathsValue)) {
        throw ErrorInfo(Tr::tr("Scan script must not return searchPaths; use the Scanner.searchPaths "
                               "script instead."),
                        script.location());
    }
    ...
    if (scanResult.scannerProperties.isEmpty()) {
        throw ErrorInfo(Tr::tr("Scan script object must contain scannerProperties."),
                        script.location());
    }
