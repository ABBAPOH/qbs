...
void UtilitiesExtension::setupStaticMethods(JSContext *ctx, JSValue classObj)
{
    ...
    setupMethod(ctx, classObj, "includedMocCppBaseNames",
                      &UtilitiesExtension::js_includedMocCppBaseNames, 2);
}
...
JSValue UtilitiesExtension::js_includedMocCppBaseNames(JSContext *ctx, JSValueConst,
                                                        int argc, JSValueConst *argv)
{
    try {
        if (argc < 2) {
