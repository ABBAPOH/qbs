...
void TestBlackbox::installLocations_data()
{
    ...
    QTest::newRow("explicit values, using items modules")
        << QString("bindir") << QString("dlldir") << QString("libdir") << QString("pluginDir")
...
void TestBlackbox::installLocations()
{
    ...
    if (!binDir.isEmpty()) {
        const auto prop = useModule
                              ? (useInstallPaths ? "modules.installpaths.bin:"
                                                 : "modules.items.application.installDir:")
                              : "products.theapp.installDir:";
        params.arguments.push_back(prop + binDir);
    }
    if (!dllDir.isEmpty()) {
        const auto prop = useModule
                              ? (useInstallPaths ? "modules.installpaths.lib:"
                                                 : "modules.items.library.installDir:")
                              : "products.thelib.installDir:";
        params.arguments.push_back(prop + dllDir);
    }
    if (!libDir.isEmpty()) {
        const auto prop = useModule ? "modules.items.library.importLibInstallDir:"
    ...
    if (!pluginDir.isEmpty()) {
        const auto prop = useModule
                              ? (useInstallPaths ? "modules.installpaths.plugins:"
                                                 : "modules.items.plugin.installDir:")
    ...
    if (!dsymDir.isEmpty()) {
        if (useModule) {
            params.arguments.push_back(
                "modules.items.application.debugInformationInstallDir:" + dsymDir);
            params.arguments.push_back(
                "modules.items.library.debugInformationInstallDir:" + dsymDir);
            params.arguments.push_back(
                "modules.items.plugin.debugInformationInstallDir:" + dsymDir);
        } else {
    ...
    const BinaryInfo plugin = {
        ...
        : isDarwin ? "theplugin"
                   : "libtheplugin.so",
        pluginDir.isEmpty() ? (isDarwin ? "PlugIns" : "/lib/install-locations/plugins/")
                            : pluginDir,
